...
public:
    ...
    explicit BaseDate(const Date &date, const WEEKDAY wd = WEEKDAY::UNKNOWN) : m_date(date),
                                                                               m_weekday(wd)
    {
    }
...
{
public:
    UTCTime(const Date &date, const Time &time) : BaseDate(date),
                                                  m_time(time)
    {
    }
...
public:
    UNIXTime(const Date &date, const int secs) : BaseDate(date),
                                                 m_secs(secs)
    {
    }
    ...
    UNIXTime() : BaseDate({.m_year = -1, .m_month = -1, .m_day = -1}),
...
{
public:
    explicit DateTime(const Time &time) : m_time(time)
    {
    }
